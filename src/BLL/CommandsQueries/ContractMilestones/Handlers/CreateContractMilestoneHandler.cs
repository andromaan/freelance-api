...
{
    public async Task<Result<ContractMilestoneVM?>> HandleAsync(ContractMilestone? entity,
        CreateContractMilestoneVM createModel, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<ContractMilestoneVM?>.NotFound($"Contract with Id {createModel.ContractId} not found");
        }

        if (existingContract.CreatedBy != userId && userRole != Settings.Roles.AdminRole)
        {
            return Result<ContractMilestoneVM?>.Unauthorized("You are not authorized to create a milestone for this contract");
        }
        ...
        {
            return Result<ContractMilestoneVM?>.GetResponse(
