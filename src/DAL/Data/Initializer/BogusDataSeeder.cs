...
public static class BogusDataSeeder
{
    public static async Task SeedTestDataAsync(AppDbContext context)
    {
        ...
        // Generating moderators
        var moderatorUsers = await GenerateModeratorsAsync(context, defaultPasswordHash, moderatorRole, countries);

        // 2. Users
        var freelancersUsers = await GenerateFreelancersAsync(context, faker, defaultPasswordHash, freelancerRole,
            countries, skills, languages);
    ...
    private static async Task<List<User>> GenerateModeratorsAsync(AppDbContext context,
        string defaultPasswordHash, Role moderatorRole, List<Country> countries)
    {
        var modFaker = new Faker<User>("en")
            ...
            .RuleFor(u => u.ModifiedBy, (_, u) => u.Id)
    ...
    private static async Task<List<User>> GenerateFreelancersAsync(AppDbContext context, Faker faker,
        string defaultPasswordHash, Role freelancerRole, List<Country> countries, List<Skill> skills,
        List<Language> languages)
    {
        ...
        // Generating languages for freelancer
        foreach (var user in freelancersUsers)
        {
            ...
            foreach (var l in userLangs)
            {
                context.UserLanguages.Add(new UserLanguage
                    { UserId = user.Id, LanguageId = l.Id, ProficiencyLevel = faker.PickRandom<ProficiencyLevel>() });
            }
        }
        ...
        // Generating profile for freelancer
    ...
    private static async Task<List<User>> GenerateEmployersAsync(AppDbContext context, Faker faker,
        string defaultPasswordHash, Role employerRole, List<Country> countries, List<Language> languages)
    {
    ...
    private static async Task<List<Contract>> GenerateContractsAsync(AppDbContext context, Faker faker,
        List<Project> projects,
        List<User> freelancersUsers, List<User> employerUsers)
    {
        ...
        var contractFaker = new Faker<Contract>("en")
            ...
            .RuleFor(c => c.Status,
                (_, c) => inProgressProjects.First(x => x.Id == c.ProjectId).Status == ProjectStatus.Completed
    ...
    private static async Task GenerateReviewsAsync(AppDbContext context, Faker faker, List<Contract> contracts,
        int freelancerRoleId, int employerRoleId)
    {
    ...
    private static async Task GeneratePaymentsAndTransactionsAsync(AppDbContext context, Faker faker,
    ...
    private static async Task GenerateDisputesAsync(AppDbContext context, Faker faker, List<Contract> contracts,
    ...
    private static async Task GenerateNotificationsAsync(AppDbContext context, Faker faker, List<User> freelancers,
        List<User> employers)
    {
